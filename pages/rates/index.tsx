  ...
  const search = (input: string) => {
    setFilteredRates(
      rates.filter((rate: Rate) => {
        ...
        return prepareTextSearch(rateString).includes(prepareTextSearch(input));
      })
    );
  };
            ...
            <div>
              ...
              <Row>
                <Col>
                  <p>
                    Rate-Type is{" "}
                    <b>{RatePropsByType[selected.rateType].title}.</b>
                  </p>
                </Col>
              </Row>
              ...
              <Table responsive>
                <thead>
                  <tr>
                    ...
                    <th>{RatePropsByType[selected.rateType].colName1}</th>

                    <th>{RatePropsByType[selected.rateType].colName2}</th>

                    <th>{RatePropsByType[selected.rateType].colName3}</th>
                  </tr>
                </thead>
                ...
                <tbody>
                  <tr>
                    ...
                    {RatePropsByType[selected.rateType].roll.map((prop) => (
                      <td key={prop}>{selected[prop]!.toFixed(2)}</td>
                    ))}
                  </tr>
                  ...
                  <tr>
                    ...
                    {RatePropsByType[selected.rateType].pitch.map((prop) => (
                      <td key={prop}>{selected[prop]!.toFixed(2)}</td>
                    ))}
                  </tr>
                  ...
                  <tr>
                    ...
                    {RatePropsByType[selected.rateType].yaw.map((prop) => (
                      <td key={prop}>{selected[prop]!.toFixed(2)}</td>
                    ))}
                  </tr>
                </tbody>
